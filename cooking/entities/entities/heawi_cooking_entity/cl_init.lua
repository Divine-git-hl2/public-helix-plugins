...
function ENT:Draw()
    ...
    if trace.Entity == self and ply:GetPos():Distance(self:GetPos()) < 120 then
        ...
        cam.Start3D2D(self:GetPos() + offsetVector, ang, 0.1)
            draw.SimpleText("COOKING STOVE", font, -78, -30, colorBlack, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            draw.SimpleText("COOKING STOVE", font, -80, -32, colorWhite, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        cam.End3D2D()
    end
end